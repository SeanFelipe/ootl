...
class Pacman:
    ...
    def get_dots_xy(self):
        dots_xy_array = []
        r = self.get_rect()  # get our current x/y/w/h values
        dots_xy_array.append(r.midtop)
        dots_xy_array.append(r.midbottom)
        dots_xy_array.append(r.midleft)
        dots_xy_array.append(r.midright)
        return dots_xy_array
...
while running:
    ...
    dots_xys = pacman.get_dots_ahead(2)
    for dxy in dots_xys:
        pygame.draw.circle(screen, DOT_COLOR, dxy, DOT_SIZE)

    # dots_xys is an array
    # how do we iterate through an array in Python?
    # we use a loop. What kind of loop?

    # loop through array
        # with each iteration, draw a dot
